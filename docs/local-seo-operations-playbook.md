...
- Run `pnpm performance:audit` after a production build before major deploys. It keeps deployed HTML, CSS, JS, image weight, hero loading priority, and third-party asset boundaries healthy for visitors and search crawlers.
...
- Keep real Wayside photos in `public/images/` and listed in `image-sitemap.xml` through `src/pages/image-sitemap.xml.ts` so search engines can discover actual church imagery, not just a generic preview card. Keep titles, captions, and the Charlton image location metadata accurate.
- Keep full-size source photos in `source-assets/original-images/` instead of `public/images/`. Anything under `public/` is deployed, so use compressed WebP or appropriately sized JPG/PNG assets there.
...
- Keep recent teaching discoverable through generated `/teaching/.../` watch pages, related-message links, `video-sitemap.xml`, and `teaching-feed.xml`, all generated from the same YouTube feed used by the Teaching and Sermons pages. Staff should only need to upload the sermon to YouTube.
- Keep XML sitemap `lastmod` values honest. Static pages should reflect the current Git commit date, and generated teaching pages should reflect YouTube publish dates rather than keyword-driven priority or changefreq values.
- Keep `/llms.txt` generated from site settings and the YouTube feed so AI tools see current service times, contact details, visitor FAQ answers, public leadership facts, official pages, machine-readable resources, and recent teaching pages.
...
- Keep Event schema times precise. Sunday Worship and recurring ministry events should publish local start and end dates with explicit Eastern timezone offsets and `isAccessibleForFree: true`.
